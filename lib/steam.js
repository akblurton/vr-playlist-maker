...
module.exports.getInstalledApps = async function getInstalledApps(paths) {
  ...
  return await Promise.all(manifests.map(async function([library, manifest]) {
    const { appid: id, name } = parse(
      await readFile(manifest, "utf-8")
    ).AppState;
    return {
      id,
      name,
...
async function getAppDetails(app, force) {
  ...
  console.log(get(entry, "entries.common"));
...
module.exports.launchApp = async function(app, isVR) {
  const { id, exe } = app;
  ...
  return async function() {
    ...
    if (isVR) {
      await require("taskkill")("vrmonitor.exe", {
        force: true,
      });
    }
  };
