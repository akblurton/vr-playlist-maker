...
module.exports.launchApp = async function(app) {
  ...
  spawn(steam, [`steam://launch/${id}`], {
    detached: true,
    shell: false,
  });
  return async function() {
    return new Promise((reject, resolve) => {
      console.log(escapeStringRegexp(exe));
      ps.lookup({}, (err, results) => {

        fs.writeFileSync("ps.json", JSON.stringify(results, null, 2));
      });
      ps.lookup({
        command: path.basename(exe),
