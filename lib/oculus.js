...
let oculusNames = null;
async function getAllAppNames(force = false) {
  if (process.platform !== "win32") {
    return {};
  }
  if (oculusNames && !force) {
    return oculusNames;
  }
  ...
  const target = path.resolve("./oculusdb.sqlite");
      ...
      function(err) {
        ...
        db.each(
          ...
          function(err, row) {
            ...
            const endRemoved = b.slice(0,
              b.indexOf(
                new Buffer([0x72, 0x08, 0x00, 0x00])
              )
            );
